...
export function CameraProctor({ active, onVideoRef, onMediaEvent }: CameraProctorProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Video Preview */}
      <div className="relative overflow-hidden rounded-lg border-2 border-indigo-500 bg-black">
        <video
          ref={videoRef}
          autoPlay
          muted
        ...
        <div className="absolute bottom-2 left-2 flex flex-col gap-1">
          ...
          <div
            className={`flex items-center gap-1 rounded px-2 py-1 text-xs font-medium ${
              micActive ? 'bg-emerald-500/90 text-white' : 'bg-red-500/90 text-white'
            }`}
          >
      ...
      {(cameraError || micError) && (
        <div className="space-y-2">
          {cameraError && (
            <div className="flex items-start gap-2 rounded-lg border border-red-300 bg-red-50 p-3 dark:border-red-900 dark:bg-red-950/30">
